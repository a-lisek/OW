% Funkcja wykorzystujaca zmodyfikowany algorytm naiwny do odnalezienia punktow
% niezdominowanych
% Zwracane:
...
function [PSet indicator dominated count] = getNonDominated_Naive2(X, dir)
    a = size(X);
    ...
    for ii=1:a(2)
       ...
       % Porownujemy z reszta punktow jesli okazalo sie ze punkty
       % niezominowane nie dominuja punktu ii
       if (isDominating ==1 )
            for i = 1:size(X,2)
